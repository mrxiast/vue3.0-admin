...
const whiteList = ['/login', '/forgot-password', '/register'] // no redirect whitelist
router.beforeEach(async (to, from, next) => {
  ...
  let hasToken = settings.isNeedLogin ? '66666' : 'temp_token'
  if (hasToken) {
    if (to.path === '/login') {
      ...
      if (isGetUserInfo) {
        ...
        try {
          ...
          if (settings.isNeedLogin) {
            ...
            //去掉验证登录
            accessRoutes = await store.dispatch('permission/generateRoutes', ['admin'])
          } else {
