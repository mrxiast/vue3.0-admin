    ...
    <div class="top-box">
      <el-card>
        <div class="filtter-box">
          <div class="input-box">
            <el-input v-model="userId" placeholder="Enter user ID or account ID" />
          </div>
          <div class="input-box">
            <el-input v-model="referrerName" placeholder="Enter Referrer Name" />
          </div>
    ...
    <div class="table-box">
      <el-card>
        <el-table v-loading="listLoading" :data="tableList" element-loading-text="Loading" border highlight-current-row>
          ...
          <el-table-column prop="name" label="FP Account ID" align="center" />
          <el-table-column prop="mobile" label="Referral Code" align="center" />
          <el-table-column prop="agentId" label="Mobile Number" align="center" />
          <el-table-column align="center" label="Registration Date"></el-table-column>
          <el-table-column prop="createdAt" label="Account Level" align="center" />
          <el-table-column prop="createdAt" label="Status" align="center" />
          <el-table-column prop="createdAt" label="My Referrer" align="center" />
          <el-table-column label="Action" align="center">
            <template #default="scope">
              <el-dropdown trigger="click">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item class="action-item" @click="goDetail(scope.row)">Check details</el-dropdown-item>
                    <el-dropdown-item class="action-item" @click="setActive(scope.row)">
                      {{ scope.row.status === '1' ? 'Block' : 'Active' }}
                    </el-dropdown-item>
                    <el-dropdown-item class="action-item" @click="goCashIn(scope.row)">Cash In</el-dropdown-item>
                    <el-dropdown-item class="action-item" @click="">Transactions</el-dropdown-item>
                    <el-dropdown-item class="action-item" @click="">Friends</el-dropdown-item>
                    <el-dropdown-item class="action-item" @click="goComission(scope.row)">Commission</el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
            </template>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const goDetail = (row) => {
      $router.push('/customer/customer-detail/123')
    }
    const setActive = (item) => {
      state.showRightBox = true
    ...
    const goCashIn = (row) => {
      $router.push({ path: '/transactions/cash-in/customer-cash-in', query: { id: row.id } })
    }
    return {
      ...toRefs(state),
      search,
